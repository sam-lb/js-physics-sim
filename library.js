      ...
      for (let j=i+1; j<this.objects.length; j++) {
        ...
        if (obj1.type === "RigidBody" && obj2.type === "RigidBody") {
		  ...
		  if (this.universalGravityEnabled) { obj1.gravitate(obj2); }
			...
			//if (p5.Vector.sub(obj1.pos, obj2.pos).mag() < 10) {
			if (obj1.collideRect(obj1, obj2)) {
				// obj1.applyForce(p5.Vector.sub(obj1.pos, obj2.pos).mult(obj2.mass*obj2.vel.mag()/0.1));
				// obj2.applyForce(p5.Vector.sub(obj2.pos, obj1.pos).mult(obj1.mass*obj1.vel.mag()/0.1));
				// let a = obj1.angVel;
				//obj1.pos.sub(p5.Vector.mult(obj1.vel, env.timeStep));
				//obj2.pos.sub(p5.Vector.mult(obj2.vel, env.timeStep));
				//obj1.angle-=obj1.angVel*env.timeStep;
				//obj2.angle-=obj2.angVel*env.timeStep;
				// obj1.setAngularVelocity(obj2.angVel);
				// obj2.setAngularVelocity(a);
				obj1.collisionRespond(obj2);
			} else {
			}
		}
      }
...
class RigidBody {
  ...
  applyTorque(torque) {
	  // calculate the angular acceleration caused by a torque using alpha = t/m
	  this.angAcc += torque / this.mass;
  }
...
class Rectangle extends RigidBody {
	...
	constructor(x=0, y=0, width=1, height=1, mass=1, charge=0, color=createVector(255, 0, 0),
				elasticity=1, csf=0.75, ckf=0.5, dragCoef=0.5, fixed=false) {
		super(env, x, y, mass, charge, color, elasticity, csf, ckf, dragCoef);
		this.type = "Rectangle";
  ...
  collideRect(rect1, rect2) {
	  ...
	  let p1, p2;
	  return rect1.normals.concat(rect2.normals).every(function(norm) {
		  p1 = rect1.minMax(norm);
		  p2 = rect2.minMax(norm);
		  return (((p1.min <= p2.max) && (p1.max >= p2.min) || (p2.min >= p1.max) && (p2.max >= p1.min)));
	  });
  }
  
  collisionRespond(rect) {
	  /* let A, B, K, L, D, a, b, c;
	  
	  L = this.moment * this.angVel + crossProduct2d(this.centerOfMass, this.getMomentum()) + rect.moment * rect.angVel + crossProduct2d(rect.centerOfMass, rect.getMomentum());
	  
	  A = this.mass * this.vel.x + rect.mass * rect.vel.x;
	  B = this.mass * this.vel.x * this.vel.x + rect.mass * rect.vel.x * rect.vel.x;
	  if (Math.sign(this.vel.x) === -1 && Math.sign(rect.vel.x) === 1) {
		this.vel.x = solveQuadratic(this.mass * this.mass + this.mass * rect.mass, -2*A*this.mass, A*A-rect.mass*B);
	  } else {
		this.vel.x = solveQuadraticN(this.mass * this.mass + this.mass * rect.mass, -2*A*this.mass, A*A-rect.mass*B);
	  }
	  rect.vel.x = (A - this.mass * this.vel.x)/rect.mass;
	  
	  A = this.mass * this.vel.y + rect.mass * rect.vel.y;
	  B = this.mass * this.vel.y * this.vel.y + rect.mass * rect.vel.y * rect.vel.y;
	  if (Math.sign(this.vel.y) === -1 && Math.sign(rect.vel.y) === 1) {
		this.vel.y = solveQuadratic(this.mass * this.mass + this.mass * rect.mass, -2*A*this.mass, A*A-rect.mass*B);
	  } else {
		this.vel.y = solveQuadraticN(this.mass * this.mass + this.mass * rect.mass, -2*A*this.mass, A*A-rect.mass*B);
	  }
	  rect.vel.y = (A - this.mass * this.vel.y)/rect.mass;
	  
	  // K = this.moment * this.angVel * this.angVel + rect.moment * rect.angVel * rect.angVel;
	  // D = L - crossProduct2d(this.centerOfMass, this.getMomentum()) - crossProduct2d(rect.centerOfMass, rect.getMomentum());
	  // a = rect.moment * rect.moment + rect.moment * this.moment;
	  // b = -2 * rect.moment * D;
	  // c = D * D - this.moment * K;
	  // rect.angVel = solveQuadratic(a, b, c);
	  // this.angVel = (D - rect.moment * rect.angVel) / this.moment;
	  //if (env.simTime > 1) {noLoop();} */
	  
	  /* FAKE COLLISION RESPONSE CODE */
	  /* F*dt = m*dv ---and---> dv = v since v0=0 (assumption)
	  F = v*m/dt
	  so also t*dt = I*w 
	  t = I*w/dt
	  w and v represent desired new vels */
	  
	  let forceAppliedToThis, forceAppliedToRect, torqueAppliedToThis, torqueAppliedToRect, totalMag, A, B;
	  
	  if (!this.fixed) {
		  this.pos.sub(p5.Vector.mult(this.vel, env.timeStep));
		  this.angle-=this.angVel*env.timeStep;
	  }
	  if (!rect.fixed) {
		  rect.pos.sub(p5.Vector.mult(rect.vel, env.timeStep));
		  rect.angle-=rect.angVel*env.timeStep;
	  }
	  
	  let test = this.pos.x < rect.pos.y;
	  ...
	  // forceAppliedToThis = p5.Vector.mult(p5.Vector.sub(rect.vel, this.vel), rect.mass / env.timeStep);
	  // forceAppliedToRect = p5.Vector.mult(p5.Vector.sub(this.vel, rect.vel), this.mass / env.timeStep);
	  // torqueAppliedToThis = this.moment * rect.angVel / env.timeStep;
	  // torqueAppliedToRect = rect.moment * this.angVel / env.timeStep;
	  // this.vel = createVector(0, 0);
	  // rect.vel = this.vel.copy();
	  // this.angVel = 0;
	  // rect.angVel = 0;
	  
	  // this.applyForce(forceAppliedToThis);
	  // this.applyTorque(torqueAppliedToThis);
	  // rect.applyForce(forceAppliedToRect);
	  // rect.applyTorque(torqueAppliedToRect);
	}
