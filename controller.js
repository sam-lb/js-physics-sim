...
function generateOrbs() {
	...
	for (let i=0; i<orbCount; i++) {
		...
		mass = random(1, 6);
		orb = new Orb(env, random() * env.xMeters, random() * env.yMeters, 2, mass, 0, createVector(mass/5*255, 128, 128), 1);
		// /*orb = (new Orb(env, pos.x, pos.y, 15, 5000, 0, createVector(255, 255, 0), 1));*/
		//orb.setVelocity(p5.Vector.fromAngle(HALF_PI+atan2(pos.y-center.y, pos.x-center.x)).mult(2));
	}
	...
	// for (let i=0; i<4; i++) {
		// new Orb(env, x, 0.5*env.yMeters, 1, .1, 0, createVector(255, random()*255, 128), 1, 0, 0, 0);
		// x += 1;
	// }
	...
	// for (let i=0; i<orbCount; i++) {
		// rect = new Rectangle(1+(env.xMeters-2)*random(0,1), 1+(env.yMeters-2)*random(0,1), random(1, 3), random(1, 3), 1, 0,
							 // createVector(random(0, 255), random(0, 255), random(0, 255)));
		// rect.setAngularVelocity(random(0, 6));
		// rect.setVelocity(createVector(random(-15, 15), random(-15, 15)));
	// }
	// createBoundaryRects();
}

function createBoundaryRects() {
	let color = createVector(200, 200, 200);
	(new Rectangle(env.xMeters/2, 0.5, env.xMeters, 1, 1, 0, color)).setFixed();
	(new Rectangle(env.xMeters/2, env.yMeters-0.5, env.xMeters, 1, 1, 0, color)).setFixed();
	(new Rectangle(0.5, env.yMeters/2, 1, env.yMeters-2, 1, 0, color)).setFixed();
	(new Rectangle(env.xMeters-0.5, env.yMeters/2, 1, env.yMeters-2, 1, 0, color)).setFixed();
}
